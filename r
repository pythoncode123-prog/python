...
def generate_region_chart(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        df_region = df.groupby(['DATE', 'FormattedDate', 'REGION'], as_index=False)['TOTAL_JOBS'].sum()
        pivot_df = df_region.pivot_table(index=['DATE', 'FormattedDate'], columns='REGION', values='TOTAL_JOBS', aggfunc='sum').fillna(0)
        
        # Make the date index available for table generation
        pivot_df = pivot_df.reset_index()
        
        # Get column headers (region names)
        region_columns = [col for col in pivot_df.columns if col not in ['DATE', 'FormattedDate']]
        
        # Generate headers for the table
        table_headers = ["<tr><th>Date</th>"] + [f"<th>{col}</th>" for col in region_columns] + ["</tr>"]
        
        # Generate table rows
        table_rows = []
        for _, row in pivot_df.iterrows():
            formatted_date = row['FormattedDate']
            cells = [f"<td>{formatted_date}</td>"]
            for col in region_columns:
                cells.append(f"<td>{int(row[col])}</td>")
            table_rows.append(f"<tr>{''.join(cells)}</tr>")
        
        # Use table-chart macro instead of chart macro for better compatibility
        return f"""
<ac:structured-macro ac:name="table-chart">
    <ac:parameter ac:name="type">bar</ac:parameter>
    <ac:parameter ac:name="title">Total Jobs by Region</ac:parameter>
    ...
    <ac:rich-text-body>
        <table class="wrapped">
            <colgroup>
                {"".join([f"<col/>" for _ in range(len(region_columns)+1)])}
            </colgroup>
            <tbody>
                <tr>
                    <th>Date</th>
                    {chr(10).join([f"<th>{col}</th>" for col in region_columns])}
                </tr>
...
def generate_daily_trend_chart(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        # Use table-chart macro with line type for better compatibility
        return f"""
<ac:structured-macro ac:name="table-chart">
    ...
    <ac:parameter ac:name="dataOrientation">vertical</ac:parameter>
    <ac:parameter ac:name="columns">Total Jobs</ac:parameter>
    <ac:rich-text-body>
        <table class="wrapped">
            <colgroup>
                <col/>
            ...
            <tbody>
                ...
                {chr(10).join([f"<tr><td>{row['FormattedDate']}</td><td>{int(row['TOTAL_JOBS'])}</td></tr>" for _, row in df_summary.iterrows()])}
            </tbody>
        </table>
    </ac:rich-text-body>
</ac:structured-macro>
...
def publish_to_confluence(report_file='task_usage_report_by_region.csv', test_mode=False, skip_actual_upload=False):
    ...
    try:
        ...
        execution_timestamp = datetime.strptime('2025-06-26 23:18:14', '%Y-%m-%d %H:%M:%S')  # Updated timestamp
