...
def generate_table_and_chart(df: pd.DataFrame, baseline: int = 1899206) -> str:
    """Generate the main table and column chart for the Confluence page."""
    try:
        ...
        chart_df['FormattedDate'] = pd.to_datetime(chart_df['DATE']).dt.strftime('%-m/%-d/%Y')
        
        # Generate table rows for the data table below the chart
        table_rows = ["|| Date || Baseline || Total Jobs || Variation ||"]
        for _, row in chart_df.iterrows():
            table_rows.append(
                f"| {row['FormattedDate']} | {int(row['Baseline'])} | {int(row['TOTAL_JOBS'])} | {int(row['Variation'])} |"
            )
        
        # Generate the Confluence column chart macro
        chart_rows = ["|| Date || Baseline || Total Jobs ||"]
        for _, row in chart_df.iterrows():
            chart_rows.append(
                f"| {row['FormattedDate']} | {int(row['Baseline'])} | {int(row['TOTAL_JOBS'])} |"
            )
...
def generate_region_chart(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        # Use simple chart macro for pie chart
        return f"""
