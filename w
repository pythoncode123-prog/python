...
def get_user_password(username):
    """Get password from user without storing it in the config file."""
    print(f"\nEnter password for {username} to connect to Confluence:")
    password = getpass.getpass()
    return password

def create_session(config: Dict, password: str = None) -> requests.Session:
    ...
    if auth_type == 'basic':
        ...
        # Otherwise use API_TOKEN if it exists
        elif 'API_TOKEN' in config:
            session.auth = HTTPBasicAuth(config['USERNAME'], config['API_TOKEN'])
        else:
            logging.error("No password or API_TOKEN provided for authentication")
            return None
    elif auth_type == 'jwt':
        ...
        session.headers.update({
            "Cookie": f"JSESSIONID={config.get('API_TOKEN', '')}"
        })
...
def publish_to_confluence(report_file='task_usage_report_by_region.csv', test_mode=False):
    ...
    try:
        ...
        # Get user password for Confluence (not stored in config)
        password = get_user_password(config['USERNAME'])
        if not password:
            print(f"[{datetime.now()}] No password provided, cannot connect to Confluence")
            if test_mode:
                print(f"[{datetime.now()}] TEST MODE: Continuing with simulation only")
            else:
                return False
        
        # Create a session with the password (not API token)
        session = create_session(config, password)
