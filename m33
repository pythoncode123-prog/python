...
def auto_discover_csv_countries(csv_path, target_month):
    ...
    {CountryCode}_Usagereport_{YYYYMM}.csv (case-insensitive)
    ...
    # Pattern: {CountryCode}_Usagereport_{YYYYMM}.csv (case-insensitive)
    pattern = f"*_*report_{target_month}.csv"  # More flexible pattern for glob
    ...
    for file_path in matching_files:
        ...
        # Extract country code using case-insensitive regex
        # Matches: MX_USAGEREPORT_202508.csv, MX_Usagereport_202508.csv, etc.
        match = re.match(r'^([A-Z]{2,3})_[Uu][Ss][Aa][Gg][Ee][Rr][Ee][Pp][Oo][Rr][Tt]_\d{6}\.csv
...
, filename, re.IGNORECASE)
        if match:
            country_code = match.group(1).upper()  # Ensure uppercase
