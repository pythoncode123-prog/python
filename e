...
def load_config(config_path: str = "config.json") -> Dict:
    ...
    try:
        ...
        required_keys = ['CONFLUENCE_URL', 'USERNAME', 'SPACE_KEY', 'PAGE_TITLE', 'CSV_FILE']
...
def create_session(config: Dict) -> requests.Session:
    ...
    if auth_type == 'basic':
        if 'PASSWORD_B64' in config:
            # Use base64 decoded password
            password = decode_password(config['PASSWORD_B64'])
            if password:
                session.auth = HTTPBasicAuth(config['USERNAME'], password)
            else:
                logging.error("Failed to decode password")
                return None
        elif 'API_TOKEN' in config:
            # Use API token if provided
            session.auth = HTTPBasicAuth(config['USERNAME'], config['API_TOKEN'])
        else:
            logging.error("No PASSWORD_B64 or API_TOKEN provided for authentication")
            return None
    elif auth_type == 'jwt':
        ...
        if 'API_TOKEN' in config:
            ...
            logging.error("No API_TOKEN provided for JWT authentication")
            return None
        ...
        if 'SESSION_COOKIE' in config:
            ...
            logging.error("No SESSION_COOKIE provided for cookie authentication")
            return None
    elif auth_type == 'kerberos':
        # For Kerberos/NTLM authentication
        try:
            from requests_kerberos import HTTPKerberosAuth
            session.auth = HTTPKerberosAuth()
        except ImportError:
            logging.error("Kerberos authentication requires requests_kerberos package")
            return None
    ...
    try:
        ...
        if not os.path.exists(config_path):
            ...
            default_config = {
                ...
                "USERNAME": "45292857",
                "PASSWORD_B64": "U0FTVTE0M3Nhc3Uq",  # Base64 encoded, replace with actual encoded password
                "AUTH_TYPE": "basic",
                "SPACE_KEY": "DIGIBAP",
        ...
        if config['AUTH_TYPE'] == 'basic' and 'PASSWORD_B64' not in config and 'API_TOKEN' not in config:
            print(f"[{datetime.now()}] ERROR: No PASSWORD_B64 or API_TOKEN provided for authentication")
            print(f"[{datetime.now()}] Please add PASSWORD_B64 (Base64 encoded password) to your config file")
            
            # Generate a sample base64 encoding as an example
            sample_password = "your_password_here"
            sample_encoded = base64.b64encode(sample_password.encode()).decode('utf-8')
            print(f"[{datetime.now()}] Example: Add this to your config.json:")
            print(f"  \"PASSWORD_B64\": \"{sample_encoded}\",")
        ...
        execution_user = config.get('EXECUTION_USER', 'satish537')
