...
def get_password_from_config(config):
    ...
    if 'PASSWORD_ENCRYPTED' in config:
        try:
            ...
            return None
    elif 'PASSWORD_B64' in config:
        try:
            return base64.b64decode(config['PASSWORD_B64']).decode('utf-8')
        except Exception as e:
            logging.error(f"Error decoding base64 password: {str(e)}")
            return None
    return None
...
def generate_table_and_chart(df: pd.DataFrame, baseline: int = 1899206) -> str:
    """Generate the main table and column chart for the Confluence page using wiki markup."""
    try:
        ...
        for _, row in df.iterrows():
            date = pd.to_datetime(row['DATE']).strftime('%-m/%-d/%Y')  # Format like 4/2/2024
            total_jobs = int(row['TOTAL_JOBS'])
            variation = baseline - total_jobs
            
            # Add data for both chart and table
            chart_data.append(f"{date}|{baseline}|{total_jobs}")
            table_rows.append(f"| {date} | {baseline} | {total_jobs} | {variation} |")
        
        # Create the chart macro with exact formatting from image
        chart_macro = (
            "{chart:type=column"
        ...
        return f"Error generating table and chart: {str(e)}"
...
def generate_region_chart(df: pd.DataFrame) -> str:
    ...
    try:
        if not {'DATE', 'REGION', 'TOTAL_JOBS'}.issubset(df.columns):
            return "Missing required columns for region chart: DATE, REGION, TOTAL_JOBS"
        ...
        region_summary = latest_data.groupby('REGION', as_index=False)['TOTAL_JOBS'].sum()
        
        # Calculate total jobs and percentage
        total_jobs = region_summary['TOTAL_JOBS'].sum()
        region_summary['Percentage'] = (region_summary['TOTAL_JOBS'] / total_jobs * 100).round(1)
        
        # Generate table rows with wiki markup
        table_rows = ["|| Region || Total Jobs ||"]
        chart_rows = []
        for _, row in region_summary.iterrows():
            table_rows.append(f"| {row['REGION']} | {int(row['TOTAL_JOBS'])} |")
            chart_rows.append(f"{row['REGION']} | {int(row['TOTAL_JOBS'])}")
        
        # Use wiki markup for chart
        chart_macro = (
            "{chart:type=pie"
...
h3. Total Jobs by Region ({int(total_jobs):,} - 100%)

{table_str}
"""
    except Exception as e:
        logging.error(f"Error generating region chart: {str(e)}")
        return f"Error generating region chart: {str(e)}"
...
def generate_daily_trend_chart(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        # Since pie charts don't work well for time series, just use a table
        return f"""
...
def publish_to_confluence(report_file='task_usage_report_by_region.csv', test_mode=False, skip_actual_upload=False):
    ...
    try:
        ...
        execution_timestamp = datetime.strptime('2025-06-26 23:50:54', '%Y-%m-%d %H:%M:%S')  # Updated timestamp
        execution_user = 'satish537'  # Updated username
