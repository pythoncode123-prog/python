...
def main():
    ...
    parser.add_argument("--countries-config", help="Path to countries JSON to run multi-country workflow")  # <-- NEW
    args = parser.parse_args()

    setup_logging()
    os.makedirs('config', exist_ok=True)

    # File existence checks (single-country mode)
    ...
    # Check for password if publishing
    ...
    # Run the workflow (single-country)
    run_workflow(
        CONFIG_FILE,
        QUERY_FILE,
        OUTPUT_CSV,
